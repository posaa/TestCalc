...
namespace CalcEngine
{
	...
	public partial class CalcEngine
	{
        ...
        {
            if (string.Compare(identifier, "NaN", true) == 0)
            {
                return "NaN";
            }
            return null;
        ...
        void GetToken()
        {
            ...
            if (isDigit || c == _decimal)
			{
                ...
                //здесь результат div - используется как количество знаков после запятой
                if (!sci)
                {
			...
			if (c == '#')
			{
                ...
                _token = new Token(DateTime.Parse(lit), TKID.ATOM, TKTYPE.LITERAL);
                return;
			}
