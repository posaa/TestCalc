...
namespace CalcEngine
{
	...
	public class Expression : IComparable<Expression>
	{
        ...
        public int CompareTo(Expression other)
        {
            ...
            var reportName = System.Configuration.ConfigurationSettings.AppSettings["ReportName"];
            ...
            if (reportName == "ПромежуточнаяОтчетностьНПФ_АО" || reportName == "ПромежуточнаяОтчетность_39_КоллИнвест"
                    || reportName == "ПромежуточнаяОтчетностьНПФ_НекоммерческаяОрганизация"
                    || reportName == "ПромежуточнаяОтчетностьНекредитОрг_Проф"
                     || reportName == "ПромежуточнаяОтчетность_9_Проф"
                      || reportName == "ПромежуточнаяОтчетностьБФО"
                      || reportName == "ПромежуточнаяБухгОтчетностьАИФ"
                      || reportName == "ПромежуточнаяОтчетностьМФК"
                        || reportName == "ПромежуточнаяОтчетностьНПФ_АО_РедУказания4381У"
                      || reportName == "ПромежуточнаяОтчетностьНПФ_НекоммерческаяОрганизация_РедУказания4381У"
                      )
            {
                if ((Math.Abs(Math.Abs((double)c1) - Math.Abs((double)c2)) == 1)|| (Math.Abs(Math.Abs((double)c1) - Math.Abs((double)c2)) == 2))
                {
                    return 0;
                }
            }
	...
	class BinaryExpression : Expression
	{
		...
		override public object Evaluate()
		{
            ...
            // handle everything else
#warning Найти другой способ определения разрядности
            //_rgt._token.decimals += 3;  //=======================================    КОСТЫЛЬ
            switch (_token.ID)
			{
				case TKID.ADD:
                    //if (_lft._token.decimals > _rgt._token.decimals)
                    //{
                    //    return Math.Round((double)_lft._token.Value + (double)_rgt._token.Value,_lft._token.decimals,MidpointRounding.AwayFromZero);
                    //}
                    //else
                    //{
                    //    return Math.Round((double)_lft._token.Value + (double)_rgt._token.Value, _rgt._token.decimals, MidpointRounding.AwayFromZero);
                    //}
				...
				case TKID.DIVINT:
                    //if (_lft._token.decimals > _rgt._token.decimals)
                    //{
                    //    return (double)(int)Math.Round((double)_lft._token.Value / (double)_rgt._token.Value, _lft._token.decimals, MidpointRounding.AwayFromZero);
                    //}
                    //else
                    //{
                    //    return (double)(int)Math.Round((double)_lft._token.Value / (double)_rgt._token.Value, _rgt._token.decimals, MidpointRounding.AwayFromZero);
                    //}
