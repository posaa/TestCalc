...
namespace CalcEngine
{
    static class MathTrig
    {
        public static void Register(CalcEngine ce)
        {
            ...
            ce.RegisterFunction("ROUNDRANGE", 1, int.MaxValue, RoundRange);       //round all values to the given decimals claimed in the last value
        ...
        {
            return Math.Round((double)p[0],(int)p[1], MidpointRounding.AwayFromZero);
        }

        static object RoundRange(List<Expression> p)
        {
            List<double> ld = new List<double>();
            int range = p.Count - 1;
            int MaxFloatLenght = int.Parse(p[range]._token.Value.ToString());
            for (int e = 0; e < range; e++)
            {
                ld.Add(Math.Round((double)p[e], MaxFloatLenght, MidpointRounding.AwayFromZero));
            }
            return ld.ToArray();
        }
